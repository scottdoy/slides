%% Silhouette Texture Feature Extraction
% Script to extract texture features for silhouette project
% 2017-02-23 SD
...
for iimg = 1:nimgs
    ...
    feats_rgb_path = fullfile(feat_dir, [base_name '_' num2str(RES_MPP) '_rgb_features.mat']);
    feats_hsv_path = fullfile(feat_dir, [base_name '_' num2str(RES_MPP) '_rgb_features.mat']);
    feats_lab_path = fullfile(feat_dir, [base_name '_' num2str(RES_MPP) '_lab_features.mat']);
...
%     if(exist(feats_rgb_path, 'file') && exist(feats_hsv_path, 'file') && exist(feats_lab_path, 'file')), fprintf(1, '\tPixel features exist, skipping...\n'); continue; end
    ...
    % Resize image by this amount
    img = double(imresize(img, 0.25/RES_MPP));
    imghsv = rgb2hsv(img);
    imglab = rgb2lab(img);
    imgdata = cat(3, img, imghsv, imglab);
    [h, w, d] = size(imgdata);
    imgdata = reshape(imgdata, h*w, d);
    
    % Load labels and resize
    load(geom_path);
...
% Save the file
pixel_path = fullfile(feat_dir, 'pixel_data.mat');
save(pixel_path, 'X', 'Y', 'IMG_RESIZE', '-v7.3');
