...
for iimg = 1:nimgs
    ...
    if(exist(feats_rgb_path, 'file'))% && exist(feats_hsv_path, 'file') && exist(feats_lab_path, 'file')),
         fprintf(1, '\tPixel features exist, skipping...\n'); 
         continue; 
    end
    ...
    if(~exist(feats_rgb_path, 'file'))
        [feats_rgb, feats_rgb_names] = get_texture_features(img);
%         fun = @(blk) silhouette_texture_blkextract(blk.data);
%         feats_rgb = blockproc(img, [10,10], fun,...
%             'UseParallel', true);
%         
